...
// PROVENANCE: the XP-per-level curve originates in Funcom's SkillXPPerLevel.json;
// the keystone map encodes purchased-keystone SP bonuses. Copied verbatim so the
// offline give-charxp recompute matches dune-admin's verified behaviour. Do not
// edit by hand — re-vendor from the pinned source if dune-admin updates.
